...
const Card = ({
 blurbId
  ...
  const platformIcon = imageMatch(blurb?.platformName || '', PLATFORM_IMAGE).src;

  const editorRef = useRef<LexicalEditor>();

  const {platformGenerationMap} = useBlurbGenerationContext();
  const {isLoading, completion} = platformGenerationMap.get(blurb?.platformName as PLATFORM) as UseCompletionHelpers || {isLoading: true, completion: ""};
  ...
  return (
    <section className="card">
      ...
      {!blurb?.isLoading && (<div className="basic_tool">
          <div className="copy">
